...
sealed class SLSizes {
  ...
  ///
  /// Одно число на все случаи: и закреплённый сайдбар на десктопе,
  /// и оверлей на планшете, и выдвижная панель на телефоне
  /// (`screens/app-shell.md`, «Адаптив»). 280 вместо прежних 240 — плата
  /// за то, чтобы поле поиска перестало быть шириной в одну фразу.
  ...
  /// Колонка даты.
  ///
  /// В MVP-экране очереди её нет: состав колонок задан в
  /// `screens/queue-issues.md`, «Строка задачи», и даты в нём нет.
  /// Значение оставлено токеном системы (`system.md`, 10.3).
  static const dateColumn = 96.0;

  /// Минимальная ширина самостоятельного поля поиска (`system.md`, 10.3.1).
  ///
  /// Ниже этой ширины плейсхолдер перестаёт помещаться, и поле начинает
  /// читаться как строка текста, а не как элемент управления.
  static const searchFieldMinWidth = 240.0;

  /// Минимальная ширина колонки названия задачи (`screens/queue-issues.md`).
  ///
  /// Это правило, а не следствие: пороги набора колонок выведены из него,
  /// а не наоборот. Сорок символов — граница, ниже которой название
  /// перестаёт быть названием.
  static const issueTitleMinWidth = 280.0;
