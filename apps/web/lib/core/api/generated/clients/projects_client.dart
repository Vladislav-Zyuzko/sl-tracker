...
abstract class ProjectsClient {
  ...
  ///
  /// Параметр `q` включает поиск по составу проекта на сервере: подстрока в имени или в email, без учёта регистра. Для селекторов автора и исполнителя нужен именно он, а не подсказка упоминаний: та ограничена по частоте, требует ключа задачи и не показывается читателям. Ограничения частоты здесь нет: поиск идёт по составу одного проекта, который спрашивающему и так виден целиком.
  ///
  /// [q] - Поиск по составу проекта: подстрока в имени или в email, без учёта регистра. Пустая строка и пробелы равнозначны отсутствию параметра. Порядок и курсор те же, что и без поиска, поэтому при переходе на следующую страницу `q` нужно передавать вместе с `cursor`.
  ///
  /// [cursor] - Курсор следующей страницы из поля `nextCursor`.
