...
export class MembersController {
  ...
  @ApiOperation({
    ...
    description:
      ...
      'Параметр `q` включает поиск по составу проекта на сервере: подстрока в имени ' +
      'или в email, без учёта регистра. Для селекторов автора и исполнителя нужен ' +
      'именно он, а не подсказка упоминаний: та ограничена по частоте, требует ключа ' +
      'задачи и не показывается читателям. Ограничения частоты здесь нет: поиск идёт ' +
      'по составу одного проекта, который спрашивающему и так виден целиком.',
  })
