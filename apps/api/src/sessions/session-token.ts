...
/**
 * Первые 8 символов предъявляемого токена — подпись для человека, а не секрет.
 *
 * Это начало идентификатора сессии, который и так ездит в каждом запросе и хранится
 * в базе открытым текстом. Он нужен единственно для того, чтобы владелец опознал свой
 * токен в списке («это тот, что в контейнере MCP»); в проверке подлинности он не
 * участвует — её делает только HMAC от верификатора.
 */
